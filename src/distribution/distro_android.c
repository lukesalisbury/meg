...
gboolean Distro_AndroidCreate( const gchar * project_title, const gchar * game_path )
{
	gchar * shared_directory = Meg_Directory_Share("binaries");
	gchar * target_name;
	gchar * zip_original_path;
	gchar * zip_target_path;
	gchar * game_target_path = "res/game.data";

	target_name = g_strdup_printf("%s", project_title );
	zip_original_path = g_build_filename( shared_directory, "android-app.zip", NULL);
	zip_target_path = g_strdup_printf( "%s"G_DIR_SEPARATOR_S"%s-android.apk", Meg_Directory_Document(), project_title );
