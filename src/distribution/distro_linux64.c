...
gboolean Distro_Linux64Create( const gchar * project_title, const gchar * game_path )
{
	...
	if ( g_file_get_contents( game_path, &game_buffer, &game_buffer_length, NULL ) )
	{
		...
		binary_original_path = g_build_filename( shared_directory, "linux64-binary.gz", NULL);
		binary_temporary_path = g_build_filename( g_get_tmp_dir(), target_name, NULL);

		zip_target_path = g_strdup_printf( "%s"G_DIR_SEPARATOR_S"%s-linux64.zip", Meg_Directory_Document(), project_title );
		...
		{
			g_warning("Distro_LinuxCreate Failed");
		}
